...
-- Bootstrap script for an EMPTY database. Not a migration.
--
-- TWO THINGS TO KNOW BEFORE RUNNING THIS ON A DATABASE THAT HAS DATA:
--
-- 1. The DROP block below is commented out. It used to be labelled "safe
--    re-run" and it is the opposite of safe: `DROP TABLE … CASCADE` on
--    price_history destroys the entire price corpus, and on profiles/bookings
--    every user and booking, with no confirmation and no backup. Nothing about
--    re-running this file is safe on a populated database. With the block
--    disabled, the CREATE TABLE statements below fail with 42P07 ("relation
--    already exists") on a database that is already set up — a loud, harmless
--    refusal instead of a silent deletion.
--
-- 2. This file does not describe the current schema. `price_history` here has 16
--    data columns; the ingest path writes 32, and the missing 16 — plus the
--    `snapshot_metadata` and `forecast_store` tables — are added by
--    `migrations/001_price_history_and_run_metadata.sql`. Rebuilding from this
--    file alone silently reverts that migration, at which point every insert the
--    scheduler attempts fails on unknown columns. If you rebuild, re-apply 001.
-- ================================================================
...
-- ================================================================
-- STEP 1: DROP EVERYTHING — DISABLED. Destroys all data. Read note 1 above.
-- To enable on a scratch database, delete the two comment-fence lines only.
-- ================================================================
/*  ---- DROP FENCE: remove this line and the matching one below to enable ----
...
DROP FUNCTION IF EXISTS public.expire_old_alerts()        CASCADE;
    ---- DROP FENCE: remove this line and the matching one above to enable ---- */
