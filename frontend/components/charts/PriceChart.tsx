...
interface Props { forecast: (ForecastPoint | ForecastDay)[]; trend: Trend | string; height?: number; }
...
export function PriceChart({ forecast, trend, height=240 }: Props) {
  ...
  const paths = useMemo(() => {
    ...
    const xTicks = forecast.map((p, i) => ({
      x: px(i),
      label: p.date ? new Date(p.date).toLocaleDateString("en-IN", { day: "numeric", month: "short" }) : (p.day === 0 ? "Today" : `Day ${p.day}`)
    })).filter((_, i) => i === 0 || (i + 1) % 3 === 0 || i === n - 1);

    const yTicks = Array.from({ length: 5 }, (_, i) => ({ value: yMin + ((yMax - yMin) * i) / 4, y: py(yMin + ((yMax - yMin) * i) / 4) }));
    const dots = forecast.map((p, i) => ({ cx: px(i), cy: py(p.price), price: p.price }));
