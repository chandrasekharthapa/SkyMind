...
const META: Record<string,{img:string;tag:string;tagClass:string}> = {
  ...
  JAI:{ img:'https://images.unsplash.com/photo-1524613032530-449a5dd58f0b?w=600&q=80&auto=format&fit=crop', tag:'Pink City', tagClass:'badge-black' },
  COK:{ img:'https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&q=80&auto=format&fit=crop', tag:"God's Own", tagClass:'badge-off' },
  AMD:{ img:'https://images.unsplash.com/photo-1599030179960-4d1b5c0fe2e0?w=600&q=80&auto=format&fit=crop', tag:'Heritage', tagClass:'badge-black' },
}
...
export default function PopularDestinations() {
  ...
  if (loading) return (
    <div className="dest-grid">
      {[0,1,2].map(i => (
        <div key={i} className="dest-card" style={{ borderRight:i<2?'1px solid var(--grey1)':'none' }}>
          <div className="dest-img-wrap"><div className="skel" style={{ height:'100%' }} /></div>
          <div className="dest-body">
            <div className="skel" style={{ height:'14px', marginBottom:'8px', width:'60%' }} />
            <div className="skel" style={{ height:'28px', width:'40%' }} />
          </div>
        </div>
      ))}
    </div>
          ...
          >
            ...
            <div className="dest-body">
              ...
              <div className="dest-meta">
                ...
                <div style={{ marginLeft:'auto', fontSize:'.7rem', color:'var(--grey3)', fontFamily:'var(--fm)' }}>avg ₹{(r.avg_price_inr/1000).toFixed(1)}k</div>
              </div>
            </div>
          </Link>
        )
