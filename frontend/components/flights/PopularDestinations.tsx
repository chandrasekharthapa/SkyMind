    ...
    const load = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select("origin_code, destination_code, dest_city, min_price_inr, avg_price_inr, flights_per_day, avg_duration_min")
          .eq("origin_code", "DEL")
          .order("flights_per_day", { ascending: false })
          .limit(3);
        if (mounted) {
          setRoutes(!error && data && data.length > 0 ? data : FALLBACK_ROUTES);
        }
      } catch {
