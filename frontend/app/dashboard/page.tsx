...
const DEMO_BOOKINGS = [
  { id:"1", booking_reference:"SKY26J4R2X", status:"CONFIRMED", origin_code:"DEL", destination_code:"DXB", total_price:28450, currency:"INR", created_at:new Date().toISOString() },
  { id:"2", booking_reference:"SKY26K1M7P", status:"PENDING",   origin_code:"BOM", destination_code:"LHR", total_price:52300, currency:"INR", created_at:new Date(Date.now()-86400000).toISOString() },
];
const DEMO_ALERTS = [
  { id:"1", origin_code:"DEL", destination_code:"DXB", departure_date:"2026-07-15", target_price:26000, last_price:24100, triggered_count:1 },
  { id:"2", origin_code:"BOM", destination_code:"SIN", departure_date:"2026-08-10", target_price:18000, last_price:22400, triggered_count:0 },
];
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setBookings(DEMO_BOOKINGS); setAlerts(DEMO_ALERTS);
        ...
        <div className="dash-header">
          <div className="wrap">
            <div className="dash-title-row">
              ...
              <Link href="/flights" className="btn-primary">+ New booking</Link>
            </div>
          </div>
        </div>
          ...
          <div style={{ marginTop:28 }}>
            <div className="dash-stats">
              {[
                ...
                { icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>, val:`₹${(totalSpent/1000).toFixed(1)}k`, label:"Total spent" },
                ...
                { icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg>, val:String(confirmedTrips*1200), label:"Points" },
              ].map(s => (
            ...
            <div className="dash-grid">
              ...
              <div>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
                  <h2 style={{ fontFamily:"var(--fd)", fontSize:"1.6rem", letterSpacing:".04em", color:"var(--black)" }}>MY TRIPS</h2>
                ...
                {!loading && bookings.length===0 && (
                  <div className="empty-state">
                    No bookings yet. <Link href="/flights" style={{ color:"var(--red)", textDecoration:"underline" }}>Search flights</Link> to get started.
                  </div>
                )}
                  ...
                  <div key={b.id} className="booking-card" style={{ borderColor:i===0?"var(--black)":undefined }} onClick={() => router.push("/booking")}>
                    ...
                    <div className="bk-body">
                      <div className="bk-route">
                        ...
                        <div className="fline" style={{ flex:1, margin:"0 12px" }}>
                          ...
                          <div className="fline-track" style={{ flex:1 }}>
                            <div className="fline-plane"><svg width="12" height="12" viewBox="0 0 24 24" fill="var(--red)"><path d="M17.8 19.2L16 11l3.5-3.5C21 6 21 4 19.5 2.5S18 2 16.5 3.5L13 7 4.8 6.2c-.5-.1-.9.1-1.1.5L2 8.9c-.2.4-.1.9.2 1.2l4.6 4.1-1.5 6.4 2.8 2.8 5.3-3.2 4.1 4.6c.3.4.8.5 1.2.2l1.1-1.2c.4-.2.6-.6.5-1.1z"/></svg></div>
                          </div>
                          <div className="fline-dot" />
                        </div>
                      ...
                      <div className="bk-foot">
                        ...
                        <span style={{ fontFamily:"var(--fd)", fontSize:"1.2rem", letterSpacing:".03em" }}>₹{Math.round(b.total_price||0).toLocaleString("en-IN")}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div>
                <div className="sidebar-card">
                  ...
                  {alerts.length===0&&!loading&&<div style={{ padding:"16px", fontSize:".8rem", color:"var(--grey3)", textAlign:"center" }}>No alerts set yet.</div>}
                  {alerts.map(a => (
                    <div key={a.id} className="alert-item">
                      <div className="alert-route">
                        {a.origin_code} <span style={{ color:"var(--grey3)" }}>→</span> {a.destination_code}
                        <span style={{ marginLeft:"auto", width:7, height:7, borderRadius:"50%", background:a.triggered_count>0?"#22c55e":"#f59e0b", display:"inline-block" }} />
                      </div>
                      ...
                      <div className="alert-prices">
                        ...
                        {a.last_price && <span style={{ color:a.triggered_count>0?"#166534":"var(--grey3)" }}>Now: ₹{Math.round(a.last_price).toLocaleString("en-IN")}</span>}
                      </div>
                      {a.triggered_count>0 && <div className="alert-hit">✓ Target reached — book now!</div>}
                    </div>
                  ))}
                  <div style={{ padding:"12px 16px" }}>
                    <Link href="/predict" className="btn-outline" style={{ width:"100%", justifyContent:"center", fontSize:".78rem", padding:"9px" }}>+ Add alert</Link>
                  </div>
                </div>
                ...
                <div className="sidebar-card">
                  ...
                  {[
                    ...
                    <Link key={q.label} href={q.href} className="qa-item" style={{ borderBottom:i<2?undefined:"none" }}>
                      <div style={{ width:28, height:28, background:"var(--off)", border:"1px solid var(--grey1)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>{q.icon}</div>
                      {q.label}
                    </Link>
                  ))}
                </div>
