  ...
  useEffect(() => {
    const obs = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) {
        ...
        const t = setInterval(() => { v = Math.min(v + step, to); setVal(Math.round(v)); if (v >= to) clearInterval(t) }, 20)
      }
    }, { threshold: .5 })
...
const TICKER = ['Amadeus GDS','Prophet ML','Supabase','Razorpay PCI-DSS','scikit-learn','FastAPI','Dijkstra Algorithm','90+ Indian Airports','Vercel Edge']
    ...
    <div>
      ...
      <div className="hero">
        ...
        <div className="hero-left a1">
          ...
          <div className="hero-stats">
            {[{to:2,suf:'M+',label:'Fares analyzed'},{to:38,suf:'%',label:'Average savings'},{to:94,suf:'%',label:'AI accuracy'}].map(s => (
        ...
        <div className="hero-right a2">
          ...
          <div style={{ marginTop:'16px', display:'flex', alignItems:'center', gap:'12px' }}>
            ...
            <span style={{ fontSize:'.7rem', color:'var(--grey3)', letterSpacing:'.06em', textTransform:'uppercase', fontFamily:'var(--fm)' }}>AI-enhanced results · Prophet ML</span>
            <div style={{ height:'1px', flex:1, background:'var(--grey2)' }} />
          </div>
        </div>
      </div>
        ...
        <div className="wrap">
          ...
          <div className="how-grid">
            <div className="how-left">
              ...
              {[
                { n:'01 —', title:'Live fare ingestion', text:'Amadeus GDS feeds pulled every 5 minutes across 90+ Indian airports and 8 international hubs.' },
                { n:'02 —', title:'ML scoring engine', text:'scikit-learn + Prophet model scores each fare: Buy Now, Wait, or Set Alert. 94% accuracy.' },
                { n:'03 —', title:'Hidden route discovery', text:"Dijkstra's algorithm evaluates 90+ hub cities to find cheaper multi-hop alternatives." },
              ].map(s => (
      ...
      <div className="feat-section">
        <div className="wrap">
          ...
          <div className="feat-grid">
            {[
              { num:'01 / INTELLIGENCE', title:'ML Price Intelligence', desc:'Trained on millions of fare datapoints. Predicts price movements with 94% accuracy using scikit-learn + Prophet.', icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg> },
              { num:'02 / ROUTING',      title:'Hidden Route Finder',   desc:"Dijkstra's shortest-path algorithm discovers cheaper multi-stop alternatives across 90+ Indian cities.", icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="5" cy="5" r="2"/><circle cx="19" cy="5" r="2"/><circle cx="19" cy="19" r="2"/><circle cx="5" cy="19" r="2"/><path d="M7 5h10M19 7v10M5 7v10M7 19h10"/></svg> },
              { num:'03 / FORECAST',     title:'30-Day Forecast',       desc:'Full price trajectory with confidence intervals. See the best and worst departure dates before you commit.', icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg> },
              { num:'04 / ALERTS',       title:'Smart Price Alerts',    desc:'Set a target price. Our AI monitors 24/7 and fires instant Email + SMS + WhatsApp the moment it\'s reached.', icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg> },
            ].map((f,i) => (
      ...
      <footer>
        <div className="footer-inner">
          ...
          <span className="footer-copy">© 2025 SkyMind · AI Flight Intelligence · Beta</span>
