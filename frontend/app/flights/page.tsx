...
function FlightsContent() {
  ...
  const doSearch = async (currentForm = form, currentSort = sort) => {
    ...
    try {
      ...
      if (currentSort === 'Departure') sorted.sort((a,b) => (a.itineraries[0]?.segments[0]?.departure_time||'').localeCompare(b.itineraries[0]?.segments[0]?.departure_time||''))
      setFlights(sorted)
    } catch(e:any) {
      const msg = e.message || ''
  ...
  const handleSort = (s: string) => {
    ...
    if (s === 'Departure') sorted.sort((a,b) => (a.itineraries[0]?.segments[0]?.departure_time||'').localeCompare(b.itineraries[0]?.segments[0]?.departure_time||''))
    setFlights(sorted)
  }
        ...
        <div className="wrap" style={{ paddingTop:'24px', paddingBottom:'60px' }}>
          ...
          {error && (
            <div style={{ border:'1px solid var(--red)', padding:'20px 24px', background:'rgba(232,25,26,.04)', marginBottom:'16px', borderLeft:'4px solid var(--red)' }}>
              ...
              <button className="btn btn-primary" onClick={()=>doSearch()} style={{ fontSize:'.78rem', padding:'8px 16px', marginTop:'4px' }}>
                Try again
              </button>
            </div>
          )}
            ...
            <div key={i} className="flight-card" style={{ overflow:'hidden', marginBottom:'8px' }}>
              <div style={{ display:'grid', gridTemplateColumns:'160px 1fr 140px' }}>
                ...
                <div style={{ padding:'20px 24px', display:'flex', alignItems:'center', gap:'16px' }}>
                  <div className="skel" style={{ height:'28px', width:'60px' }}/>
          ...
          {!loading && searched && flights.length===0 && !error && (
            <div style={{ border:'1px solid var(--grey1)', padding:'60px 24px', textAlign:'center' }}>
              ...
              <div style={{ fontSize:'.875rem', color:'var(--grey4)', marginBottom:'4px' }}>Amadeus test API has limited route coverage.</div>
              <div style={{ fontSize:'.82rem', color:'var(--grey3)', marginBottom:'20px', fontFamily:'var(--fm)' }}>Try DEL→BOM, BOM→BLR, or DEL→BLR with a date 7+ days away.</div>
              ...
              >
                    ...
                    <div className="t-mid">
                      ...
                      <div style={{ height:'1px', width:'100%', background:'var(--grey2)', position:'relative' }}>
                        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', background:'white', padding:'0 4px', color:'var(--red)' }}>
                          <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M17.8 19.2L16 11l3.5-3.5C21 6 21 4 19.5 2.5S18 2 16.5 3.5L13 7 4.8 6.2c-.5-.1-.9.1-1.1.5L2 8.9c-.2.4-.1.9.2 1.2l4.6 4.1-1.5 6.4 2.8 2.8 5.3-3.2 4.1 4.6c.3.4.8.5 1.2.2l1.1-1.2c.4-.2.6-.6.5-1.1z"/></svg>
                        </div>
                      </div>
                ...
                <div className="flight-bottom">
                  <div style={{ display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap' }}>
                    ...
                    {f.ai_insight?.reason && (
                      <span style={{ fontSize:'.75rem', color:'var(--grey4)', maxWidth:'300px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                        {f.ai_insight.reason}
                      </span>
                    )}
                  ...
                  <span style={{ fontSize:'.7rem', color:'var(--grey3)', fontFamily:'var(--fm)', whiteSpace:'nowrap' }}>
                    {form.cabin_class==='ECONOMY'?'Economy':form.cabin_class} · 15kg
                  </span>
                </div>
              </div>
            )
