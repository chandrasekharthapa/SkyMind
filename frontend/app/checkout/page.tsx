...
export default function CheckoutPage() {
  ...
  // The fare/tax split is read off the stored offer, never derived from the total.
  // These two rows were `price * 0.85` and `price * 0.15` — unconditionally, without
  // even checking whether the provider had sent a base fare — so every checkout
  // displayed an invented breakdown of the amount the user was about to pay.
  // `backend/routers/booking.py::_extract_price_details` stores NULL when the offer
  // carries no base; this is the same rule at the point of display.
  const offerPrice = (booking?.flight_offer_data as { price?: { base?: unknown } } | null | undefined)?.price;
  const rawBase = offerPrice?.base;
  const base = typeof rawBase === "number" && rawBase > 0
    ? Math.round(rawBase)
    : typeof rawBase === "string" && parseFloat(rawBase) > 0
      ? Math.round(parseFloat(rawBase))
      : null;
  const taxes = base === null ? null : Math.max(0, Math.round(price) - base);

  // Route from session data. These read `|| "DEL"` and `|| "BOM"`, which turned a
  // route the backend could not parse — stored as NULL for exactly that reason —
  // into a confident "DEL ➔ BOM" on the payment screen.
  const origin = booking?.origin_code || "—";
  const destination = booking?.destination_code || "—";
              ...
              <div className="price-panel-body">
                ...
                {base === null ? (
                  <div className="price-row" style={{ borderBottom: "none" }}>
                    <span className="price-row-label">Fare breakdown</span>
                    <span className="price-row-val" style={{ fontSize: "0.8rem", opacity: 0.6 }}>Not itemised by the airline</span>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="price-row" style={{ borderBottom: "none" }}>
                      ...
                      <span className="price-row-val">₹{(taxes ?? 0).toLocaleString("en-IN")}</span>
                    </div>
                  </>
                )}
