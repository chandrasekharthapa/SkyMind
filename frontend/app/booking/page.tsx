...
function BookingContent() {
  ...
  // A fare/tax split is a fact about the offer, not a ratio. This read
  // `flight.price.base ? … : Math.round(price * 0.85)`, so whenever the provider
  // sent no base fare — which is most Google Flights results — the page displayed
  // 85% of the total as "Base Fare" and the remaining 15% as "Taxes & Fees", both
  // invented. `backend/routers/booking.py::_extract_price_details` had the same
  // assumption and stores NULL now instead; this is the display half of that fix.
  // null means "the provider did not itemise it" and the rows say so.
  const base = flight?.price?.base ? Math.round(flight.price.base) : null;
  const taxes = base === null ? null : Math.max(0, price - base);
              ...
              {base === null ? (
                ...
                <>
                  ...
                  <div className="price-row" style={{ marginBottom: "32px" }}>
                    ...
                    <span className="price-row-val" style={{ fontSize: "0.9rem" }}>₹{(taxes ?? 0).toLocaleString("en-IN")}</span>
                  </div>
                </>
              )}
